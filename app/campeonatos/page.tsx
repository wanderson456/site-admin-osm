...
async function getLeagues(): Promise<OSMLeague[]> {
  try {
    ...
    if (!res.ok) {
      console.error(
        `Erro na API de campeonatos: status ${res.status}`
      );
      return [];
    }
    ...
    try {
      ...
      console.error(
        "A API de campeonatos não retornou JSON válido."
      );
      return [];
    }

    if (Array.isArray(data)) {
      return data as OSMLeague[];
    }

    if (data?.leagues && Array.isArray(data.leagues)) {
      return data.leagues as OSMLeague[];
    }

    if (data?.data && Array.isArray(data.data)) {
      return data.data as OSMLeague[];
    }

    console.error(
      "Formato de dados dos campeonatos não reconhecido."
    );

    return [];
  } catch (error) {
    console.error(
      "Erro ao buscar a lista de campeonatos:",
      error
    );
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 py-6">
      ...
      {leagues.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-2xl bg-card/50 text-muted-foreground">
          ...
          >
            <path
              ...
              strokeWidth="1.5"
              d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75V11.25a9 9 0 00-9-9z"
            />
          </svg>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {leagues.map((league) => {
            const leagueId =
              league.id ?? (league as any).league_id;
            ...
            return (
              ...
              >
                {/* Linha decorativa de acento na borda superior */}
