...
/**
 * Formata o valor monetário bruto para o formato compacto do OSM
 * Ex: € 8.2M, € 500K
 */
...
/**
 * Identifica a categoria do setor:
 * 1 = GOL
 * 2 = DEF
 * 3 = MEI
 * 4 = ATA
...
/**
 * Retorna a sigla da posição específica do jogador.
 */
...
function parsePositionStyle(category: number): string {
  switch (category) {
    case 1:
...
async function getPlayers(
  ...
  try {
    ...
    if (!res.ok) {
      return {
        players: [],
        error: `Erro HTTP ${res.status} da API externa.`,
      };
    }
    ...
    if (data?.players && Array.isArray(data.players)) {
      return {
        players: data.players as OSMPlayer[],
        error: null,
      };
    }
    ...
    if (data?.data && Array.isArray(data.data)) {
      return {
        players: data.data as OSMPlayer[],
        error: null,
      };
    }
    ...
    return {
      players: [],
      error: err?.message || "Erro de conexão com a API.",
    };
  }
}
...
export default async function TeamPlayersPage({
    ...
    return (
      <div className="max-w-2xl mx-auto my-12 p-6 border border-red-500/30 rounded-xl bg-red-950/20 text-white space-y-4">
        <h2 className="text-xl font-bold text-red-400">
          Falha ao identificar os parâmetros da rota
        </h2>
  ...
  const teamName =
    players[0]?.team_name || "Elenco do Time";
        ...
        <div className="space-y-10">
          {sections.map((section) => {
            const groupPlayers = players
              .filter(
                (p) =>
                  getPlayerCategory(p) === section.code
              )
              .sort((a, b) => {
                const ovrA =
                  a.main_rating ?? a.rating ?? 0;

                const ovrB =
                  b.main_rating ?? b.rating ?? 0;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <h2
                    className={`text-lg font-bold px-3 py-1 rounded-lg border ${section.color}`}
                  >
                    {section.title}
                  </h2>
